...
namespace V1000_Param_Prog
{
    public partial class frmMain : Form
    {
        ...
        private void bwrkLoadVFDDefs_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                dgvVFDParamView.Rows.Add(new string[]
                {
        ...
        private void dgVFDParamView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            String def_cell_val_str = (String)dgvVFDParamView.Rows[index].Cells[3].Value;
            Single def_cell_val;
            ...
            try
            {
                chng_cell_val_str = (String)dgvVFDParamView.Rows[index].Cells[4].Value;
            ...
            {
                MessageBox.Show("Entry Error!!");
                dgvVFDParamView.Rows[index].Cells[4].Value = ((float)VFD_Vals[index].ParamVal / VFD_Vals[index].Multiplier).ToString();
                return;
            }
            ...
            if (chng_param_val != VFD_Vals[index].ParamVal)
            {
                ...
                DataGridViewRow ClonedRow = (DataGridViewRow)dgvVFDParamView.Rows[index].Clone();
                for (int i = 0; i < dgvVFDParamView.Rows[index].Cells.Count; i++)
                    ClonedRow.Cells[i].Value = dgvVFDParamView.Rows[index].Cells[i].Value;
                ...
                dgvVFDSchedParamChange.Rows.Add(ClonedRow);

                // Fix the user entry to be the properly formatted string from any inaccuracies in formatting by the user.
                dgvVFDParamView.Rows[index].Cells[4].Value = VFD_Sched_Chng_Vals[VFD_Sched_Chng_Vals.Count - 1].ParamValDisp;
        ...
        private void btnReadVFD_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvVFDParamView.Columns[4].ReadOnly = true;
                VFD_Modified_Vals.Clear();
                dgvVFDModifiedParams.Rows.Clear();
                ...
                btnReadVFD.Enabled = false; // disable the Read VFD button while a read is in progress.
            }
        }
        ...
        private void bwrkReadVFDVals_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            for (int i = 0; i < dgvVFDParamView.RowCount; i++)
            {
                dgvVFDParamView.Rows[i].Cells[4].Value = VFD_Vals[i].ParamValDisp;
                dgvVFDParamView.Rows[i].Cells[4].ReadOnly = false;
                ...
                if (VFD_Vals[i].ParamVal != VFD_Vals[i].DefVal)
                {
                    ...
                    DataGridViewRow ClonedRow = (DataGridViewRow)dgvVFDParamView.Rows[i].Clone();
                    ClonedRow.DefaultCellStyle.BackColor = Color.White; // don't want a custom color row for this datagridview
                    for (int j = 0; j < dgvVFDParamView.ColumnCount; j++)
                        ClonedRow.Cells[j].Value = dgvVFDParamView.Rows[i].Cells[j].Value;
                    dgvVFDModifiedParams.Rows.Add(ClonedRow);
                    ...
                    dgvVFDParamView.Rows[i].DefaultCellStyle.BackColor = Color.Yellow;
                }
                else
                {
                    ...
                    dgvVFDParamView.Rows[i].DefaultCellStyle.BackColor = Color.White;
                }
            }
            ...
            btnReadVFD.Enabled = true; // re-enable the VFD read button
        ...
        private void btnModVFD_Click(object sender, EventArgs e)
        {
            if (!bwrkModVFD.IsBusy)
            {
                ...
                btnModVFD.Enabled = false; // disable the Modify VFD button while a write is in progress.
            }
        }
